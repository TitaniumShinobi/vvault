...
class VVaultFileRepository:
    ...
    def load_bytes(self, row: dict[str, Any] | None) -> tuple[bytes, str] | None:
        ...
        if not bucket or not object_key or StorageClient is None:
            return None
        storage_key = storage_path if storage_path and "#source:" in str(object_key) else object_key
        try:
            stored = StorageClient().download_bytes(bucket=str(bucket), object_key=str(storage_key))
