...
This module is intentionally independent from the legacy Supabase client.  It
answers only from the imported OVVAULTS Postgres body and reports explicit
materialization blockers when the body does not yet contain the fields Chatty
needs.
"""
...
LOCAL_BODY_DATABASE_NAME = "vvault_body_20260504t123219z"
LOCAL_BODY_DATABASE_HOST = "127.0.0.1"
LOCAL_BODY_DATABASE_PORT = "5432"
...
def _local_body_database_url(name: str | None = None) -> str:
    body_name = (name or os.environ.get("VVAULT_BODY_DATABASE_NAME") or LOCAL_BODY_DATABASE_NAME).strip()
    if not re.match(r"^vvault_body_[a-z0-9_]+$", body_name):
        body_name = LOCAL_BODY_DATABASE_NAME
    user = os.environ.get("PGUSER") or os.environ.get("USER") or os.environ.get("LOGNAME") or "devonwoodson"
    return f"postgresql://{user}@{LOCAL_BODY_DATABASE_HOST}:{LOCAL_BODY_DATABASE_PORT}/{body_name}"


def _is_local_body_database_url(raw: str | None) -> bool:
    if not raw:
        return False
    try:
        parsed = urlparse(raw)
        ...
        return False
    database = (parsed.path or "").rsplit("/", 1)[-1]
    host = (parsed.hostname or "").lower()
    return database.startswith("vvault_body_") and host in {"", "localhost", "127.0.0.1", "::1"}
...
def database_url() -> str | None:
    ...
    if explicit:
        return explicit
    body_name = os.environ.get("VVAULT_BODY_DATABASE_NAME")
    if body_name:
        return _local_body_database_url(body_name)
    candidate = os.environ.get("DATABASE_URL")
    if _is_local_body_database_url(candidate):
        return candidate
    return _local_body_database_url()
...
def _connect():
    url = database_url()
    if not url:
        raise RuntimeError("VVAULT_BODY_DATABASE_URL or DATABASE_URL is required")
    import psycopg
...
def _transcript_rows(callsign: str) -> list[dict[str, Any]]:
    return _rows(
        """
        SELECT id, title, content, created_at, source_row_id, source_hash
        FROM transcripts
...
def list_constructs() -> BodyResult:
    ...
    try:
        rows = _rows(
            """
            SELECT id, filename, object_key, content_type, created_at, sha256
            FROM vault_files
            WHERE lower(coalesce(filename, '') || ' ' || coalesce(object_key, '')) LIKE %s
            ORDER BY created_at ASC
            """,
            ("%chat_with_%",),
        )
    ...
    for row in rows:
        path = row.get("object_key") or row.get("filename") or ""
        callsign = _construct_from_path(path)
        if not callsign:
            callsign = _construct_from_path(row.get("filename") or "")
        if not callsign:
            continue
