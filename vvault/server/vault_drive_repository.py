...
class VaultDriveRepository:
    ...
    def workspace_root(
            ...
            (
                {
                    ...
                    "name": (
                        ...
                        f" ({str(item.get('sourceRelyingPartyId'))})"
                        if callsign_count.get(str(item.get("callsign") or item.get("construct_id") or ""), 0) > 1
        ...
        payload = {
            ...
            # A raw object-key prefix is storage metadata, not a user-directory
            # authority.  In particular, a system prefix must never become an
            # accidental top-level folder.  The only roots in the ordinary
            # browser are the contracted account, instances, and library views.
