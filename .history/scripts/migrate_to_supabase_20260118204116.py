...
def migrate_text_file(supabase: Client, user_id: str, file_info: dict) -> bool:
    ...
    try:
        ...
        if result.data:
            print(f"    ✓ Text file: {file_info['filename']}")
            return True
        return False
    except Exception as e:
        print(f"    ✗ Failed: {file_info['filename']} - {e}")
        return False

def migrate_binary_file(supabase: Client, user_id: str, file_info: dict) -> bool:
    ...
    try:
        ...
        if result.data:
            print(f"    ✓ Binary file: {file_info['filename']} ({content_type})")
            return True
        return False
    except Exception as e:
        print(f"    ✗ Failed: {file_info['filename']} - {e}")
    ...
    for user_data in scan_results['users']:
        ...
        if user_id:
            ...
            for file_info in user_data['files']:
                if file_info['is_text'] and file_info['extension'] in TEXT_EXTENSIONS:
                    if migrate_text_file(supabase, user_id, file_info):
                        migrated_text += 1
                else:
                    if migrate_binary_file(supabase, user_id, file_info):
                        migrated_binary += 1
