...
1. **Storage truth**: what is actually stored in `ovvaults.vault_files` or VVAULT-native object storage; legacy object references are provenance only
...
- the migration script treated `.capsule` as a binary extension in an older migration path
