...
| `VVAULT_BODY_DB_URL` | Body database URL (PostgreSQL) | _assign_ | |
| `VVAULT_BODY_DB_SERVICE_ROLE_KEY` | Body database service role key | _assign_ | |
| `VVAULT_BODY_DB_ANON_KEY` | Body database anon key | _assign_ | |
...
2. **Create new value:** Generate a new key/token per provider (database/storage provider, Google Cloud Console, etc.). Do not reuse old value.
...
5. **Revoke old value:** In the provider (database/storage provider, Google, etc.), revoke or delete the old key/token.
...
- **VVAULT_BODY_DB_*:** Body database credentials. Rotate in the database provider. Update env and restart; then revoke old credentials.
