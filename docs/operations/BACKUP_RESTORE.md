...
The current runtime body database and VVAULT-native object storage are **not** included in this repo-directory zip. Back up Postgres (`ovvaults.*`) and S3-compatible object storage separately. Legacy Supabase exports are provenance/offboarding artifacts only.
...
6. **Legacy Supabase exports:** Restore or keep them only if an offboarding/provenance drill requires them; do not use them as runtime truth.
