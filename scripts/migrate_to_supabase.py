...
class MigrationEngine:
    ...
    def run(self):
        ...
        print("=" * 70)
        print(f"\nSource: {self.source}")
        print(f"Mode: {'DRY RUN (no changes)' if self.dry_run else 'LIVE SYNC'}")
    ...
    def _fetch_remote_state(self):
        ...
        for f in files:
            ...
            if f.get('metadata'):
                try:
                    metadata = json.loads(f['metadata']) if isinstance(f['metadata'], str) else f['metadata']
                except:
                    pass
    ...
    def _show_sync_plan(self):
        ...
        print(f"\n  SYNC PLAN:")
        print(f"  ┌─────────────────────────────────────────────────────────┐")
        print(f"  │  Files to ADD:        {len(self.to_add):>5}  ({format_size(add_size):>10})     │")
        print(f"  │  Files to UPDATE:     {len(self.to_update):>5}  ({format_size(update_size):>10})     │")
        print(f"  │  Files to SKIP:       {len(self.to_skip):>5}  (unchanged)         │")
        print(f"  │  Duplicates to REMOVE:{len(self.duplicates):>5}                       │")
        print(f"  ├─────────────────────────────────────────────────────────┤")
        print(f"  │  Estimated time: {format_time(est_seconds):>10}                        │")
        print(f"  └─────────────────────────────────────────────────────────┘")
    ...
    def _remove_duplicates(self):
        ...
        for dup in self.duplicates:
            try:
                self.supabase.table('vault_files').delete().eq('id', dup['id']).execute()
                removed += 1
                self.changes_made.append(f"REMOVED duplicate: {dup.get('filename')} (id: {dup['id']})")
                print(f"    ✓ Removed: {dup.get('filename')} (id: {dup['id'][:8]}...)")
        ...
        for file_info in all_files:
            ...
            try:
                ...
                if success:
                    ...
                    self.changes_made.append(f"{action}: {file_info['relative_path']}")
                ...
                print(f"    [{completed}/{total}] ✗ {file_info['filename']}: {e}")
    ...
    def _upload_file(self, file_info: dict, is_new: bool) -> Tuple[bool, str]:
        ...
        construct_id = get_construct_id(file_info['filename'], file_info['relative_path'])
        ...
        if is_new:
            ...
            self.supabase.table('vault_files').update(file_data).eq('id', file_info['remote_id']).execute()
            return True, f"UPDATED: {file_info['relative_path']}"
    
    def _verify_sync(self):
        result = self.supabase.table('vault_files').select('id, filename, sha256').execute()
        remote_count = len(result.data) if result.data else 0
        
        expected = len(self.local_files)
        
        if remote_count == expected:
            print(f"  ✓ Verification PASSED: {remote_count} files in Supabase match local count")
        else:
            diff = remote_count - expected
            status = "extra" if diff > 0 else "missing"
            print(f"  ⚠ Verification WARNING: {remote_count} remote vs {expected} local ({abs(diff)} {status})")
    ...
    def _show_report(self):
        ...
        print("\n" + "=" * 70)
        print("SYNC COMPLETE!")
        print(f"  Added: {len([c for c in self.changes_made if c.startswith('ADDED')])}")
        print(f"  Updated: {len([c for c in self.changes_made if c.startswith('UPDATED')])}")
        print(f"  Removed duplicates: {len([c for c in self.changes_made if c.startswith('REMOVED')])}")
...
def main():
    parser = argparse.ArgumentParser(description='VVAULT Smart Sync to Supabase')
