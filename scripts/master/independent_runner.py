...
class IndependentRunner:
        ...
        if self.state_file.exists():
            try:
                with open(self.state_file, 'r') as f:
                    ...
                    self.last_user_activity = datetime.fromisoformat(state["last_user_activity"]) if state.get("last_user_activity") else None
                    self.scheduled_tasks = state.get("scheduled_tasks", [])
                    logger.info(f"Loaded autonomous state: mode={self.mode.value}")
    ...
    def _log_autonomous_action(self, action: str, details: Dict[str, Any] = None):
    ...
    def heartbeat(self) -> Dict[str, Any]:
        ...
        heartbeat_data = {
            ...
            "mode": self.mode.value,
            "time_since_user": str(self.get_time_since_user()) if self.last_user_activity else "unknown",
            "scheduled_tasks_pending": len([t for t in self.scheduled_tasks if not t.get("completed")]),
            "status": "alive"
        }
    ...
    def schedule_self_prompt(self, 
        ...
        self._log_autonomous_action("scheduled_self_prompt", {"task_id": task_id, "execute_at": execute_at.isoformat()})
        self._save_state()
    ...
    def schedule_memory_consolidation(self, delay_minutes: int = 30) -> str:
        ...
        self.scheduled_tasks.append(task)
        self._log_autonomous_action("scheduled_memory_consolidation", {"task_id": task_id})
        self._save_state()
    ...
    def _execute_self_prompt(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._log_autonomous_action("executing_self_prompt", {
            "task_id": task["id"],
    ...
    def _execute_memory_consolidation(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """Execute memory consolidation task."""
        self._log_autonomous_action("executing_memory_consolidation", {"task_id": task["id"]})
    ...
    def start_background(self, check_interval_seconds: int = 60):
        ...
        thread = threading.Thread(
            target=self.autonomous_loop,
            args=(check_interval_seconds,),
            daemon=True
        )
        thread.start()
        self._threads.append(thread)
        logger.info(f"Background autonomous loop started for {self.construct_id}")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_heartbeat": self.last_heartbeat.isoformat() if self.last_heartbeat else None,
            "last_user_activity": self.last_user_activity.isoformat() if self.last_user_activity else None,
            "time_since_user": str(self.get_time_since_user()) if self.last_user_activity else None,
            "pending_tasks": len([t for t in self.scheduled_tasks if not t.get("completed")]),
            "total_scheduled": len(self.scheduled_tasks),
            "recent_actions": self.autonomous_log[-10:]
        }


def create_independent_runner(construct_id: str, vvault_root: str, user_id: str) -> IndependentRunner:
