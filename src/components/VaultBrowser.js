...
const parseVaultLocation = (pathname, search) => {
  ...
  // Keeping it first-class prevents the workspace preview from being replaced
  // by an unrelated legacy listing after the user opens it.
...
const vaultLocationForDriveFolder = (constructId, nodeId = 'root', sourceScope = 'vvault') => (
  nodeId === 'root'
    ? `/vault/instances/${encodeURIComponent(constructId)}?sourceScope=${encodeURIComponent(sourceScope)}`
    : `/vault/folders/${encodeURIComponent(nodeId)}?constructId=${encodeURIComponent(constructId)}&sourceScope=${encodeURIComponent(sourceScope)}`
);
...
const VaultBrowser = ({ user }) => {
  ...
  const fetchFiles = useCallback(async (legacyPath = []) => {
    setLoading(true);
    setError(null);
    try {
      const normalizedPath = Array.isArray(legacyPath) ? legacyPath.filter(Boolean) : [];
      const isSystemProjection = normalizedPath[0] === 'system';
      const params = new URLSearchParams();
      if (normalizedPath.length) params.set('path', normalizedPath.join('/'));
      if (isSystemProjection) params.set('scope', 'system');
      const response = await authFetch(`/api/vault/files${params.size ? `?${params.toString()}` : ''}`);
      const data = await response.json();
      if (data.success) {
        setFiles(data.files || []);
        if (data.user_root) {
  ...
  const fetchDriveChildren = useCallback(async ({ constructId, nodeId, sourceScope = 'vvault' }) => {
    if (!constructId) return;
    setDriveState((previous) => ({ ...previous, loading: true, error: null }));
    try {
      const params = new URLSearchParams({ constructId, parentNodeId: nodeId || 'root', sourceScope });
  ...
  useEffect(() => {
    ...
    else if (!['drive', 'home', 'instances'].includes(routeState.mode)) fetchFiles(routeState.legacyPath);
  }, [fetchUserInfo, fetchFiles, fetchConstructs, fetchTrash, fetchWorkspaceRoot, routeState.mode, routeState.legacyPath]);
  ...
  useEffect(() => {
    if (routeState.mode === 'drive' && routeState.constructId) {
      setSelectedNodeIds([]);
      setNodeMenuId(null);
      fetchDriveChildren({ constructId: routeState.constructId, nodeId: routeState.nodeId || 'root', sourceScope: routeState.sourceScope });
    }
  ...
  const triggerMemupSync = async (constructId) => {
    ...
    try {
      ...
      setSyncResult(data);
      if (data.success) {
        fetchFiles(routeState.legacyPath);
      }
    } catch (err) {
  ...
  const handleUploadFiles = async (fileList, { folderUpload = false } = {}) => {
    ...
    try {
      ...
      if (data.success) {
        if (routeState.mode === 'drive') {
          fetchDriveChildren({ constructId, nodeId: routeState.nodeId || 'root', sourceScope: routeState.sourceScope });
        } else {
          fetchFiles(routeState.legacyPath);
        }
      }
  ...
  const navigateToFolder = (folder) => {
    ...
    if (routeState.mode === 'drive' && folder?.nodeId) {
      navigate(vaultLocationForDriveFolder(routeState.constructId, folder.nodeId, routeState.sourceScope));
    } else if (['home', 'instances', 'my-ai-files'].includes(routeState.mode) && folder?.constructId) {
      navigate(vaultLocationForDriveFolder(folder.constructId, 'root', folder.sourceRelyingPartyId));
  ...
  const navigateToBreadcrumb = (index) => {
    ...
    if (routeState.mode === 'drive') {
      ...
      else if (index === 1) navigate(vaultLocationForDriveFolder(routeState.constructId, 'root', routeState.sourceScope));
      else {
        const breadcrumb = driveState.breadcrumbs[index - 2];
        if (breadcrumb?.nodeId) navigate(vaultLocationForDriveFolder(routeState.constructId, breadcrumb.nodeId, routeState.sourceScope));
      }
  ...
  const createFolder = async (event) => {
    ...
    try {
      ...
      await fetchDriveChildren({ constructId, nodeId: routeState.nodeId || 'root', sourceScope: routeState.sourceScope });
    } catch (err) {
  ...
  const moveDriveNode = async () => {
    ...
    try {
      ...
      setMoveTarget(null);
      await fetchDriveChildren({ constructId: routeState.constructId, nodeId: routeState.nodeId || 'root', sourceScope: routeState.sourceScope });
    } catch (err) {
          ...
          <div className="file-list">
            ...
            {routeState.mode === 'drive' && driveState.error && (
              <div className="empty-folder drive-folder-error">
                ...
                <button onClick={() => fetchDriveChildren({ constructId: routeState.constructId, nodeId: routeState.nodeId || 'root', sourceScope: routeState.sourceScope })}>Retry</button>
              </div>
            )}
