...
def test_database_url_uses_local_body_fallback_and_ignores_remote_database_url(monkeypatch):
    monkeypatch.delenv("VVAULT_BODY_DATABASE_URL", raising=False)
    monkeypatch.delenv("VVAULT_BODY_DATABASE_NAME", raising=False)
    monkeypatch.setenv("DATABASE_URL", "postgresql://postgres:secret@db.example.supabase.co:5432/postgres")
    monkeypatch.setenv("USER", "devonwoodson")

    url = body.database_url()

    assert url == "postgresql://devonwoodson@127.0.0.1:5432/vvault_body_20260504t123219z"
    assert "secret" not in body.source_database_name(url)
    assert body.source_database_name(url) == "vvault_body_20260504t123219z"


def test_database_url_uses_named_local_body_database(monkeypatch):
    monkeypatch.delenv("VVAULT_BODY_DATABASE_URL", raising=False)
    monkeypatch.setenv("VVAULT_BODY_DATABASE_NAME", "vvault_body_custom")
    monkeypatch.setenv("USER", "devonwoodson")

    assert body.database_url() == "postgresql://devonwoodson@127.0.0.1:5432/vvault_body_custom"
    assert body.source_database_name() == "vvault_body_custom"


def test_construct_list_is_body_native_without_supabase(monkeypatch):
...
def test_construct_file_inventory_is_body_native_without_supabase(monkeypatch):
...
def test_chatty_construct_routes_answer_from_body_when_supabase_unavailable(monkeypatch):
    ...
    assert not hasattr(server, "supabase_client")


def test_chatty_read_content_routes_answer_from_body_without_supabase(monkeypatch):
    server.app.config["TESTING"] = True
    client = server.app.test_client()
    monkeypatch.delenv("VVAULT_SERVICE_TOKEN", raising=False)
...
def test_chatty_write_content_routes_use_body_service_without_supabase(monkeypatch):
    server.app.config["TESTING"] = True
    client = server.app.test_client()
    monkeypatch.delenv("VVAULT_SERVICE_TOKEN", raising=False)
