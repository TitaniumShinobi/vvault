...
from vvault_frontend_proxy import VVaultFrontendProxy  # noqa: E402
...
class TestVvaultRuntimeCutoverStatic(unittest.TestCase):
    ...
    def test_health_and_ready_use_vvault_body_runtime_authority(self):
        ...
        self.assertIn("chatty_body_service._connect()", self.server)
        ...
        self.assertIn("body_database", health_route)
        self.assertIn("body_database", ready_route)
        self.assertIn("runtime_status[\"body_database\"]", health_route)
        self.assertIn("runtime_status[\"body_database\"]", ready_route)
        self.assertIn("200 if ready else 503", self.server)

    def test_body_database_service_is_runtime_import(self):
        self.assertIn("import chatty_body_service", self.server)
        self.assertIn("import vvault_auth_repository", self.server)
        self.assertIn("import vvault_file_repository", self.server)
        self.assertIn("def _body_database_dependency_status", self.server)

    def test_launcher_uses_current_backend_health_flow(self):
        self.assertIn("BACKEND_HEALTH_URL", self.launcher)
        self.assertIn("/api/health", self.launcher)
        self.assertIn("VVAULT_BODY_DATABASE_URL", self.launcher)
    ...
    def test_frontend_script_survives_detached_launcher_stdin(self):
        ...
        self.assertIn("</dev/null", self.launcher)

    def test_frontend_proxy_resolves_public_html_documents_before_spa_fallback(self):
        expected = (
            "vvault-terms.html",
        ...
        proxy = object.__new__(VVaultFrontendProxy)
        proxy.dist_dir = REPO_ROOT / "dist"
        proxy.repo_dir = REPO_ROOT
        for filename in expected:
            self.assertEqual(proxy._static_candidate(filename), REPO_ROOT / "html" / filename)
...
class TestVvaultRuntimeCutoverRoutes(unittest.TestCase):
    ...
    def test_ready_route_uses_body_database(self):
        ...
        self.assertIn("body_database", connected_payload)
    ...
    def test_vault_service_health_reports_vvault_native_dependencies(self):
        ...
        self.assertIn("body_database", payload)
        self.assertIn("vvault_body", payload["storage_mode"])
    ...
    def test_system_file_write_is_local_transaction(self):
        ...
        self.assertTrue(payload["success"])
        self.assertEqual(payload["storage_mode"], "vvault_body")
        self.assertIn("storage_mode", payload)

    def test_memup_sync_uses_local_repository_path(self):
        with patch.object(
            server,
            "db_get_session",
            return_value={
        ...
        self.assertEqual(response.get_json()["storage_mode"], "vvault_body")
